...
class CdnFile(TLObject):
    ...
    def to_dict(self):
        return {"_": "CdnFile", "bytes": self.bytes}
...
class CdnFileReuploadNeeded(TLObject):
    ...
    def to_dict(self):
        return {"_": "CdnFileReuploadNeeded", "request_token": self.request_token}
    ...
    def to_dict(self):
        return {
            ...
            if self.file_hashes is None
                ...
                x.to_dict() if isinstance(x, TLObject) else x for x in self.file_hashes
            ],
        }
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                b"\x15\xc4\xb5\x1c",
                struct.pack("<i", len(self.file_hashes)),
                b"".join(x._bytes() for x in self.file_hashes),
            )
        )
...
class WebFile(TLObject):
    ...
    def to_dict(self):
        return {
            ...
            if isinstance(self.file_type, TLObject)
            else self.file_type,
            "mtime": self.mtime,
            "bytes": self.bytes,
        }
