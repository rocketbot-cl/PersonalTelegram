...
class GetMessageStatsRequest(TLRequest):
    ...
    def __init__(
        self, channel: "TypeInputChannel", msg_id: int, dark: Optional[bool] = None
    ):
        """
...
class LoadAsyncGraphRequest(TLRequest):
    ...
    def to_dict(self):
        return {"_": "LoadAsyncGraphRequest", "token": self.token, "x": self.x}
    ...
    def _bytes(self):
        return b"".join(
            (
                ...
                b""
                if self.x is None or self.x is False
                else (struct.pack("<q", self.x)),
            )
        )
